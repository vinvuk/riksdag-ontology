...
""")
    ...
    # Sub-diagrams — embed SVGs generated by generate_diagrams.py
    _title_map = {
        "people": "People", "orgs": "Organizations", "memberships": "Memberships",
        "elections": "Elections",
        "hierarchy": "Core document types", "motions": "Motion subtypes",
        "legal": "Legal publications", "debates": "Debates and speeches",
        "core": "Procedure model", "stages": "Legislative stages",
        "model": "Voting model",
    }
    svg_files = sorted(DOCS_DIR.glob(f"{module_id}-*.svg"))
    if svg_files:
        parts.append('<h2>Diagrams</h2>')
        parts.append('<p class="meta">Hollow triangles (△) = subclass. '
                     'Blue arrows = object properties. '
                     'Dashed boxes = external standards.</p>')
        for sf in svg_files:
            suffix = sf.stem.replace(f"{module_id}-", "")
            title = _title_map.get(suffix, suffix.replace("-", " ").title())
            parts.append(f'<h3>{esc(title)}</h3>')
            parts.append(f'<div class="diagram"><img src="{sf.name}" alt="{esc(title)}"></div>')
